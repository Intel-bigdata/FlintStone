...
SELECT COUNT(*), W_HOURS, P_PNUM, W_PNUM, P_CITY FROM WORKS CROSS JOIN PROJ GROUP BY W_HOURS, P_PNUM, W_PNUM, P_CITY HAVING W_PNUM = P_PNUM AND P_CITY IS NOT NULL AND EXISTS (SELECT P_BUDGET FROM PROJ WHERE P_PNUM = W_PNUM);
