...
SELECT COUNT(*), S_CITY FROM STAFF WHERE S_EMPNAME LIKE '___%' AND S_EMPNUM NOT LIKE '_3' GROUP BY S_CITY HAVING COUNT(*) NOT IN (SELECT COUNT(*) FROM STAFF, WORKS GROUP BY S_GRADE HAVING S_GRADE BETWEEN 11 AND (SELECT MAX(S_GRADE) FROM STAFF WHERE S_EMPNAME > 'AA') AND S_GRADE BETWEEN 12 AND 18);
