...
SELECT AVG(W_HOURS), W_EMPNUM FROM WORKS WHERE W_PNUM BETWEEN 'P1 ' AND (SELECT MAX(P_PNUM) FROM PROJ WHERE P_BUDGET > 3000.32 AND P_CITY IN (SELECT S_CITY FROM STAFF WHERE S_GRADE <> 13)) GROUP BY W_EMPNUM;
