...
SELECT P_PTYPE, P_PNAME, P_ENDDATE, P_BUDGET FROM PROJ WHERE P_CITY IS NOT NULL AND NOT EXISTS (SELECT W_EMPNUM FROM WORKS WHERE P_PNAME IS NOT NULL AND W_EMPNUM IS NOT NULL AND P_PNAME = W_EMPNUM AND P_BUDGET IS NOT NULL);
