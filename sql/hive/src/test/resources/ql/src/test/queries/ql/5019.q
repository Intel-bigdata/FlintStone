...
select p_city, p_ptype, sum(case when p_budget>10000.0 then 0.0 else p_budget end) from proj where p_pname like '%A%' group by p_city, p_ptype having p_ptype not like '%e';
