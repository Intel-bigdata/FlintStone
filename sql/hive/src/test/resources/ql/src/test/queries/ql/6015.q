...
SELECT COUNT(*), S_CITY FROM STAFF WHERE EXISTS (SELECT W_HOURS FROM WORKS WHERE W_EMPNUM = S_EMPNUM) AND S_EMPNUM > 'E1 ' GROUP BY S_CITY HAVING NOT EXISTS (SELECT S_CITY FROM STAFF WHERE NOT EXISTS (SELECT * FROM STAFF WHERE S_EMPNUM > 'E8 ') AND S_EMPNUM > 'E1 ' GROUP BY S_CITY HAVING EXISTS (SELECT W_EMPNUM FROM WORKS WHERE W_PNUM > 'E1 ' AND EXISTS (SELECT * FROM STAFF WHERE S_EMPNUM > 'E3 ' AND S_GRADE BETWEEN 11 AND 16) GROUP BY W_EMPNUM));
