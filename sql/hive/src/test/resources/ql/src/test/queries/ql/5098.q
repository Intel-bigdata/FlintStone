...
select p_city, p_ptype, sum(case when p_budget>10000.0 then 0.0 else p_budget end) from proj group by p_city, p_ptype having p_ptype not like '%e' order by p_ptype, sum(case when p_budget>10000.0 then 0.0 else p_budget end);
