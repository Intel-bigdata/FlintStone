...
SELECT AVG(W_HOURS), W_EMPNUM FROM WORKS WHERE W_PNUM > 'E1 ' AND EXISTS (SELECT * FROM STAFF WHERE S_EMPNUM > 'E3 ' AND S_GRADE BETWEEN 11 AND 16 AND S_EMPNUM = W_EMPNUM) GROUP BY W_EMPNUM HAVING W_EMPNUM IN (SELECT S_EMPNUM FROM STAFF WHERE S_EMPNUM = W_EMPNUM AND S_CITY LIKE '%e%');
