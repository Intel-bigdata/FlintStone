...
select s_empnum from staff where s_empnum in (select w_empnum from works where w_pnum in (select p_pnum from proj where p_ptype <> 'AD') or exists (select * from proj where p_pnum = w_pnum)) and s_grade > (select max(s_grade) from staff where s_grade<15);
