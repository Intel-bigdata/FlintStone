...
SELECT AVG(S_GRADE), COUNT(S_GRADE), S_EMPNUM FROM STAFF GROUP BY S_EMPNUM HAVING S_EMPNUM NOT IN (SELECT W_EMPNUM FROM WORKS WHERE NOT EXISTS (SELECT P_ENDDATE FROM PROJ WHERE P_STARTDATE > DATE '2015-09-09'));
