...
SELECT COUNT(*), W_HOURS, P_PNUM, P_PNAME, P_STARTDATE, W_PNUM FROM WORKS CROSS JOIN PROJ GROUP BY W_HOURS, P_PNUM, P_PNAME, P_STARTDATE, W_PNUM HAVING W_PNUM = P_PNUM AND P_STARTDATE > DATE '2009-02-03' AND P_PNAME NOT IN (SELECT S_EMPNAME FROM STAFF WHERE P_PNUM = S_EMPNUM);
