...
SELECT AVG(S_GRADE), COUNT(S_GRADE) FROM STAFF WHERE S_EMPNUM NOT IN (SELECT W_EMPNUM FROM WORKS WHERE W_HOURS > 30 AND EXISTS (SELECT P_BUDGET, W_HOURS FROM PROJ, WORKS WHERE W_PNUM = P_PNUM AND W_EMPNUM NOT IN ('E1 ', 'E2 ', 'E3 ', 'E4 ', 'E5 ', 'E6 ', 'E7 ', 'E8 ') GROUP BY P_CITY)) AND EXISTS (SELECT W_HOURS FROM WORKS WHERE W_EMPNUM = S_EMPNUM) AND S_CITY IS NOT NULL;
