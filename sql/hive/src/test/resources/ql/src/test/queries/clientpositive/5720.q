...
SELECT COUNT(*), S_CITY FROM STAFF WHERE S_GRADE NOT BETWEEN 11 AND (SELECT MAX(S_GRADE) FROM STAFF WHERE S_EMPNAME > (SELECT MIN(W_EMPNUM) FROM WORKS WHERE W_HOURS > 30.0)) GROUP BY S_CITY;
