...
SELECT COUNT(*) FROM WORKS RIGHT OUTER JOIN PROJ ON P_PNUM=W_PNUM WHERE P_CITY IS NOT NULL AND EXISTS (SELECT P_BUDGET FROM PROJ WHERE P_PNUM = W_PNUM AND P_ENDDATE IN (SELECT P_ENDDATE FROM PROJ WHERE P_CITY <> 'ABC'));
