...
SELECT COUNT(*) FROM STAFF, WORKS GROUP BY S_EMPNUM HAVING 'E1 ' < S_EMPNUM OR NOT EXISTS (SELECT * FROM STAFF WHERE S_EMPNUM < (SELECT MIN(W_EMPNUM) FROM WORKS WHERE W_PNUM = 'P4 ' OR W_PNUM = 'P5 ' AND W_HOURS IN (20, 40, 80)));
