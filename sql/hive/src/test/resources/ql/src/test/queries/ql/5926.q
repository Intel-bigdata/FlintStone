...
SELECT COUNT(*) FROM STAFF GROUP BY S_GRADE HAVING S_GRADE = (SELECT MAX(S_GRADE) FROM STAFF, WORKS WHERE S_EMPNUM > W_EMPNUM AND S_GRADE < (SELECT MIN(W_HOURS) FROM WORKS));
