...
SELECT COUNT(*), W_HOURS, P_CITY, W_PNUM FROM WORKS CROSS JOIN PROJ WHERE W_PNUM = P_PNUM GROUP BY W_HOURS, P_CITY, W_PNUM HAVING P_CITY IS NOT NULL AND EXISTS (SELECT P_BUDGET FROM PROJ WHERE P_PNUM = W_PNUM AND P_ENDDATE IN (SELECT P_ENDDATE FROM PROJ WHERE P_CITY <> 'ABC'));
