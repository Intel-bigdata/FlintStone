...
SELECT COUNT(*) FROM WORKS JOIN PROJ ON P_PNUM=W_PNUM WHERE W_PNUM LIKE '_6%' AND W_EMPNUM NOT LIKE '%1%' AND P_BUDGET > (SELECT AVG(P_BUDGET) FROM PROJ WHERE P_PNUM = W_PNUM OR P_CITY = (SELECT MIN(S_CITY) FROM STAFF WHERE W_EMPNUM > S_EMPNUM));
