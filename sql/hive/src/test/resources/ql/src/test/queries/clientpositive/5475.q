...
SELECT * FROM STAFF, WORKS WHERE S_EMPNUM = W_EMPNUM AND S_EMPNUM IN (SELECT W_EMPNUM FROM WORKS WHERE EXISTS (SELECT P_BUDGET, P_CITY FROM PROJ WHERE P_CITY NOT IN ('AA', 'BB') GROUP BY P_CITY, P_BUDGET HAVING P_CITY < 'Y2'));
