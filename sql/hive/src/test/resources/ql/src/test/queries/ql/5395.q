...
SELECT COUNT(*), W_HOURS FROM WORKS CROSS JOIN PROJ WHERE W_PNUM = P_PNUM AND P_CITY IS NOT NULL AND EXISTS (SELECT P_BUDGET FROM PROJ WHERE W_HOURS != 40 AND P_PNUM = W_PNUM) GROUP BY W_HOURS;
