...
SELECT AVG(W_HOURS), W_EMPNUM FROM WORKS WHERE W_PNUM > 'E1 ' AND EXISTS (SELECT * FROM STAFF WHERE S_EMPNUM > 'E3' AND S_GRADE BETWEEN 11 AND 16 AND S_EMPNUM > W_EMPNUM OR W_HOURS = (SELECT MIN(W_HOURS) FROM WORKS WHERE W_EMPNUM < S_EMPNUM)) GROUP BY W_EMPNUM;
