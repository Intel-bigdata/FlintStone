...
SELECT COUNT(*), S_CITY FROM STAFF WHERE 'E1 ' < S_EMPNUM OR NOT EXISTS (SELECT * FROM STAFF WHERE S_EMPNUM > 'E8 ') GROUP BY S_CITY HAVING COUNT(*) < (SELECT MIN(S_GRADE) FROM STAFF WHERE S_CITY < 'F4');
