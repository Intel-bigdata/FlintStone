...
SELECT COUNT(S_GRADE) FROM STAFF, WORKS WHERE S_EMPNUM = W_EMPNUM AND S_EMPNAME LIKE '___%' AND S_EMPNUM NOT LIKE '_3 ' AND W_HOURS = (SELECT MIN(W_HOURS) FROM WORKS WHERE W_PNUM IN (SELECT P_PNUM FROM PROJ LEFT OUTER JOIN STAFF ON P_CITY <> S_CITY WHERE S_EMPNUM != 'E3 ' AND S_EMPNUM != 'E4 '));
