...
SELECT AVG(W_HOURS+P_BUDGET/100), W_EMPNUM FROM WORKS, PROJ WHERE W_PNUM = P_PNUM AND P_CITY IS NOT NULL AND EXISTS (SELECT P_BUDGET FROM PROJ WHERE P_PNUM = W_PNUM) GROUP BY W_EMPNUM;
