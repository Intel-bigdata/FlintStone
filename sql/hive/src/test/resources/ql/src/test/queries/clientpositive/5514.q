...
SELECT * FROM WORKS LEFT OUTER JOIN PROJ ON P_PNUM=W_PNUM WHERE P_STARTDATE > DATE '2010-02-03' AND W_EMPNUM IN (SELECT S_EMPNUM FROM STAFF WHERE P_PNUM <> S_EMPNUM AND NOT EXISTS (SELECT * FROM WORKS WHERE W_EMPNUM LIKE '%9%_' AND W_HOURS < 13));
