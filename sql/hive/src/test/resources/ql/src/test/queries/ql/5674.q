...
SELECT S_GRADE+3*W_HOURS FROM STAFF, WORKS WHERE S_EMPNUM = W_EMPNUM AND 'E1 ' < S_EMPNUM OR NOT EXISTS (SELECT * FROM STAFF WHERE S_EMPNUM < (SELECT MIN(W_EMPNUM) FROM WORKS WHERE W_PNUM = 'P4 ' OR W_PNUM = 'P5 ' AND W_HOURS IN (20, 40, 80)));
