...
SELECT COUNT(*) FROM STAFF, WORKS GROUP BY S_CITY, S_GRADE HAVING S_CITY IS NOT NULL AND S_GRADE IS NOT NULL AND S_CITY IN (SELECT P_CITY FROM PROJ WHERE P_PNUM NOT IN (SELECT W_PNUM FROM WORKS WHERE W_HOURS > 20 AND W_EMPNUM != 'E5 '));
