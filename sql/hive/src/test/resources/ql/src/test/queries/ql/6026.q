...
SELECT COUNT(*), S_CITY, S_EMPNUM FROM STAFF WHERE (SELECT MIN(W_HOURS) FROM WORKS) > S_GRADE GROUP BY S_CITY, S_EMPNUM HAVING EXISTS (SELECT * FROM STAFF WHERE EXISTS (SELECT S_EMPNUM FROM STAFF WHERE S_EMPNUM <> 'E7 ')) AND S_EMPNUM > 'E1 ';
