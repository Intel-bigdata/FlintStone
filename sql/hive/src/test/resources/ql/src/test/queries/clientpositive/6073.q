...
SELECT COUNT(*), S_CITY, S_EMPNUM FROM STAFF WHERE S_GRADE > (SELECT MIN(W_HOURS) FROM WORKS WHERE EXISTS (SELECT P_BUDGET FROM PROJ WHERE P_BUDGET BETWEEN 20000.33 AND 80033.24)) GROUP BY S_CITY, S_EMPNUM HAVING S_EMPNUM <> 'E1 ' AND EXISTS (SELECT * FROM STAFF WHERE S_EMPNUM > 'E8 ');
