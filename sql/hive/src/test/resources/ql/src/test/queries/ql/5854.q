...
SELECT AVG(S_GRADE), COUNT(S_GRADE), S_EMPNUM, S_CITY FROM STAFF GROUP BY S_EMPNUM, S_CITY HAVING S_EMPNUM NOT IN (SELECT W_EMPNUM FROM WORKS WHERE W_HOURS > 30) AND EXISTS (SELECT W_HOURS FROM WORKS WHERE W_EMPNUM = S_EMPNUM) AND S_CITY IS NOT NULL;
