...
SELECT AVG(W_HOURS), W_EMPNUM, W_PNUM FROM WORKS WHERE W_PNUM BETWEEN 'P1 ' AND (SELECT MAX(P_PNUM) FROM PROJ WHERE P_BUDGET > 3000.32 AND P_CITY IN (SELECT S_CITY FROM STAFF WHERE S_GRADE <> 13)) GROUP BY W_EMPNUM, W_PNUM HAVING W_PNUM NOT LIKE '_6%' AND W_EMPNUM NOT LIKE '%1%' AND MAX(W_HOURS) < (SELECT AVG(P_BUDGET) FROM PROJ WHERE P_PNUM = W_PNUM OR P_CITY = (SELECT MIN(S_CITY) FROM STAFF WHERE W_EMPNUM > S_EMPNUM));
