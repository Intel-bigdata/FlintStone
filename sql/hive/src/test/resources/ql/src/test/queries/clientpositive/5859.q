...
SELECT AVG(S_GRADE), COUNT(S_GRADE), S_EMPNUM, S_EMPNAME, S_GRADE FROM STAFF GROUP BY S_EMPNUM, S_EMPNAME, S_GRADE HAVING S_EMPNAME LIKE '___%' AND S_EMPNUM NOT LIKE '_3 ' AND S_GRADE BETWEEN 11 AND (SELECT MAX(S_GRADE) FROM STAFF WHERE S_EMPNAME > 'AA');
