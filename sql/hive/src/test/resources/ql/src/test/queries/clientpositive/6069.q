...
SELECT COUNT(*), S_CITY, S_GRADE FROM STAFF WHERE S_GRADE BETWEEN (SELECT AVG(S_GRADE) FROM STAFF WHERE S_EMPNAME > (SELECT MIN(W_EMPNUM) FROM WORKS WHERE W_HOURS BETWEEN 0 AND 100)) AND 18 GROUP BY S_CITY, S_GRADE HAVING S_GRADE BETWEEN 11 AND (SELECT MAX(S_GRADE) FROM STAFF WHERE S_EMPNAME > 'AA') AND S_GRADE BETWEEN 12 AND 18;
