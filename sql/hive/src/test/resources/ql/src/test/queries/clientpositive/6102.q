...
SELECT AVG(W_HOURS+P_BUDGET/100), W_EMPNUM, P_PNUM FROM WORKS RIGHT OUTER JOIN PROJ ON P_PNUM=W_PNUM WHERE P_CITY IS NOT NULL AND EXISTS (SELECT P_BUDGET FROM PROJ WHERE P_PNUM = W_PNUM AND P_ENDDATE IN (SELECT P_ENDDATE FROM PROJ WHERE P_CITY <> 'ABC')) GROUP BY W_EMPNUM, P_PNUM HAVING W_EMPNUM IN (SELECT S_EMPNUM FROM STAFF WHERE P_PNUM <> S_EMPNUM);
