...
SELECT COUNT(*) FROM STAFF, WORKS GROUP BY S_GRADE, W_PNUM HAVING S_GRADE BETWEEN 11 AND (SELECT MAX(S_GRADE) FROM STAFF WHERE S_EMPNAME > 'AA' AND S_CITY IN (SELECT P_CITY FROM PROJ WHERE P_PNUM <> W_PNUM)) AND S_GRADE BETWEEN 12 AND 18;
