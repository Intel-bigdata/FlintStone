...
select p_pnum from proj where p_pnum in (select min(w_pnum) from works w1 where exists (select w_pnum from works w2 where w2.w_pnum=w1.w_pnum) group by w_empnum having w_empnum not in (select s_empnum from staff where s_grade>10) or exists(select s_empnum from staff where s_empnum = w_empnum));
