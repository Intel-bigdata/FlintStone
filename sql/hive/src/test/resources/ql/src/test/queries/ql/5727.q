...
SELECT COUNT(*), S_CITY FROM STAFF WHERE S_GRADE = (SELECT MAX(S_GRADE) FROM STAFF, WORKS WHERE S_EMPNUM > W_EMPNUM AND S_GRADE < (SELECT MIN(W_HOURS) FROM WORKS)) GROUP BY S_CITY;
