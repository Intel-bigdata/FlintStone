...
SELECT COUNT(*) FROM STAFF WHERE S_CITY IS NULL AND S_GRADE IS NOT NULL OR S_GRADE IN (SELECT MIN(S_GRADE) FROM STAFF WHERE EXISTS (SELECT W_HOURS FROM WORKS WHERE W_EMPNUM = 'E3 ')GROUP BY S_CITY);
