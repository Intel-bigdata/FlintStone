...
SELECT AVG(W_HOURS), W_EMPNUM FROM WORKS WHERE EXISTS (SELECT S_GRADE FROM STAFF WHERE W_EMPNUM = S_EMPNUM AND S_CITY NOT LIKE (SELECT MIN(P_CITY) FROM PROJ)) AND EXISTS (SELECT S_CITY FROM STAFF WHERE S_EMPNUM > 'E1 ') GROUP BY W_EMPNUM HAVING W_EMPNUM IN (SELECT S_EMPNUM FROM STAFF WHERE S_EMPNUM = W_EMPNUM AND S_CITY LIKE '%e%');
