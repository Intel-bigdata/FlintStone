...
SELECT * FROM WORKS JOIN PROJ ON P_PNUM=W_PNUM WHERE P_CITY IN ('Deale          ', 'Vienna         ', 'Akron          ', 'Toyko          ') AND P_PTYPE NOT IN (SELECT P_PTYPE FROM PROJ WHERE P_CITY NOT IN (SELECT S_CITY FROM STAFF WHERE S_GRADE <> 13));
