...
SELECT P_BUDGET*2/(P_BUDGET/3) FROM PROJ WHERE P_CITY IS NOT NULL AND EXISTS (SELECT P_BUDGET FROM PROJ, WORKS WHERE P_PNUM = W_PNUM AND P_ENDDATE IN (SELECT P_ENDDATE FROM PROJ WHERE P_CITY <> 'ABC'));
