...
SELECT * FROM STAFF, WORKS WHERE S_EMPNUM = W_EMPNUM AND S_EMPNUM NOT IN (SELECT W_EMPNUM FROM WORKS WHERE W_HOURS > (SELECT MAX(S_GRADE) FROM STAFF WHERE S_EMPNUM > W_EMPNUM AND S_CITY NOT LIKE 'D__%'));
