...
SELECT COUNT(*), S_CITY FROM STAFF WHERE EXISTS (SELECT W_HOURS FROM WORKS WHERE W_EMPNUM = S_EMPNUM AND W_HOURS > (SELECT MAX(S_GRADE) FROM STAFF)) AND S_EMPNUM > 'E2 ' GROUP BY S_CITY;
