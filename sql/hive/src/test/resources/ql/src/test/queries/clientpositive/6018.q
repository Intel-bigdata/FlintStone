...
SELECT COUNT(*), S_CITY, S_GRADE, S_EMPNUM, S_EMPNAME FROM STAFF WHERE S_EMPNUM NOT IN (SELECT W_EMPNUM FROM WORKS WHERE W_HOURS > 30) AND NOT EXISTS (SELECT W_HOURS FROM WORKS WHERE W_EMPNUM = S_EMPNUM) GROUP BY S_CITY, S_GRADE,S_EMPNAME, S_EMPNUM HAVING S_EMPNAME LIKE '___%' AND S_EMPNUM NOT LIKE '_3 ' AND S_GRADE BETWEEN 11 AND (SELECT MAX(S_GRADE) FROM STAFF WHERE S_EMPNAME IN (SELECT MIN(S_EMPNAME) FROM STAFF WHERE S_GRADE > 10 GROUP BY S_CITY HAVING MAX(S_EMPNAME) > 'E2 '));
