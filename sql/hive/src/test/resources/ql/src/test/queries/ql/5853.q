...
SELECT AVG(S_GRADE), COUNT(S_GRADE), S_EMPNUM FROM STAFF GROUP BY S_EMPNUM HAVING S_EMPNUM NOT IN (SELECT W_EMPNUM FROM WORKS WHERE W_HOURS > 30) AND EXISTS (SELECT W_HOURS FROM WORKS WHERE W_EMPNUM = S_EMPNUM);
