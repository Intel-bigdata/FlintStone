...
SELECT AVG(W_HOURS), W_EMPNUM FROM WORKS WHERE W_PNUM LIKE '%2%' AND W_HOURS IN (SELECT W_HOURS FROM WORKS WHERE W_EMPNUM > (SELECT MIN(S_EMPNUM) FROM STAFF WHERE S_EMPNAME > 'C1 ')) GROUP BY W_EMPNUM;
