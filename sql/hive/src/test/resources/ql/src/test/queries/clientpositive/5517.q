...
SELECT * FROM WORKS LEFT OUTER JOIN PROJ ON P_PNUM=W_PNUM WHERE P_ENDDATE BETWEEN (SELECT MAX(P_ENDDATE) FROM PROJ WHERE P_PNUM < 'P6 ') AND DATE '2015-05-06' AND NOT EXISTS (SELECT S_GRADE+3*W_HOURS FROM STAFF, WORKS WHERE S_EMPNUM = W_EMPNUM AND S_EMPNAME LIKE '___%' AND S_EMPNUM = W_EMPNUM AND S_EMPNUM LIKE '_3 ' AND S_GRADE BETWEEN 11 AND (SELECT MIN(S_GRADE) FROM STAFF WHERE S_EMPNAME > 'AA'));
