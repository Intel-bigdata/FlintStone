...
select s_empnum, sum(p_budget) from staff join (select p_budget, p_city as pp_city, p_enddate as pp_enddate from proj where substr(p_city, 1, 2) not in (select substr(p_city, 1, 2) from proj where p_city not like 'T%')) pproj on s_city = pp_city where s_grade>10 and pp_enddate< date '2011-12-13' group by s_empnum;
