...
SELECT COUNT(*) FROM STAFF GROUP BY S_EMPNUM HAVING EXISTS (SELECT W_HOURS FROM WORKS WHERE W_EMPNUM = S_EMPNUM AND W_HOURS > (SELECT MAX(S_GRADE) FROM STAFF)) AND S_EMPNUM > 'E2 ';
