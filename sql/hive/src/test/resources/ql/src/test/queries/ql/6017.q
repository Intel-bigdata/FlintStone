...
SELECT COUNT(*), S_CITY, S_GRADE FROM STAFF WHERE S_EMPNUM IN (SELECT W_EMPNUM FROM WORKS) GROUP BY S_CITY, S_GRADE HAVING S_GRADE BETWEEN 11 AND (SELECT MAX(S_GRADE) FROM STAFF WHERE S_EMPNAME > 'AA' AND S_CITY IN (SELECT P_CITY FROM PROJ)) AND S_GRADE BETWEEN 12 AND 18;
