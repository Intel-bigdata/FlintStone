...
SELECT COUNT(*), S_CITY FROM STAFF WHERE 'E1 ' < S_EMPNUM OR NOT EXISTS (SELECT * FROM STAFF WHERE S_EMPNUM IN (SELECT S_EMPNUM FROM STAFF WHERE S_EMPNUM IN ('E1 ', 'E2 ', 'E3 ', 'E4 ', 'E5 '))) GROUP BY S_CITY HAVING EXISTS (SELECT W_HOURS, W_EMPNUM FROM WORKS WHERE W_PNUM > 'E1 ' AND EXISTS (SELECT * FROM STAFF WHERE S_EMPNUM > 'E3 ' AND S_GRADE BETWEEN 11 AND 16) GROUP BY W_HOURS, W_EMPNUM);
