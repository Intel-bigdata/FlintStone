...
SELECT AVG(W_HOURS+P_BUDGET/100), W_EMPNUM, W_PNUM FROM WORKS, PROJ WHERE W_PNUM = P_PNUM AND P_ENDDATE BETWEEN DATE '2007-05-06' AND (SELECT MAX(P_ENDDATE) FROM PROJ WHERE P_PNUM < 'P6 ') GROUP BY W_EMPNUM, W_PNUM HAVING W_PNUM > 'E1 ' AND EXISTS (SELECT * FROM STAFF WHERE S_EMPNUM > 'E3 ' AND S_GRADE BETWEEN 11 AND 16 AND S_EMPNUM > W_EMPNUM);
