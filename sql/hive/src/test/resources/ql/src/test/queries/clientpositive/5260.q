...
SELECT COUNT(*) FROM WORKS INNER JOIN PROJ ON P_PNUM=W_PNUM WHERE P_CITY IS NOT NULL AND NOT EXISTS (SELECT P_BUDGET FROM PROJ WHERE P_PNAME IS NOT NULL AND W_EMPNUM IS NOT NULL AND P_PNAME = W_EMPNUM AND P_BUDGET IS NOT NULL);
