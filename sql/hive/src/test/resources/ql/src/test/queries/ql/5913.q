...
SELECT COUNT(*) FROM STAFF GROUP BY S_EMPNUM HAVING S_EMPNUM IN (SELECT W_EMPNUM FROM WORKS WHERE W_EMPNUM NOT IN (SELECT S_EMPNUM FROM STAFF WHERE S_CITY NOT IN ('AA','BB','Deale          ')));
