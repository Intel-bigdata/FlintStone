...
SELECT * FROM WORKS CROSS JOIN PROJ WHERE W_PNUM = P_PNUM AND P_ENDDATE BETWEEN DATE '2007-05-06' AND (SELECT MAX(P_ENDDATE) FROM PROJ WHERE P_PNUM < 'P6 ') AND EXISTS (SELECT * FROM WORKS LEFT OUTER JOIN PROJ ON P_PNUM=W_PNUM WHERE P_STARTDATE > DATE '2009-02-03' AND P_PNAME NOT IN (SELECT S_EMPNAME FROM STAFF WHERE P_PNUM = S_EMPNUM));
