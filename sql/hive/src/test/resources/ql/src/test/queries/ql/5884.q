...
SELECT COUNT(*), W_HOURS, W_PNUM, P_BUDGET, W_EMPNUM FROM WORKS JOIN PROJ ON P_PNUM=W_PNUM GROUP BY W_HOURS, W_PNUM, W_EMPNUM, P_BUDGET HAVING W_PNUM LIKE '_6%' AND W_EMPNUM NOT LIKE '%1%' AND P_BUDGET = (SELECT AVG(P_BUDGET) FROM PROJ WHERE P_PNUM = W_PNUM);
