...
SELECT COUNT(*), S_CITY, S_GRADE FROM STAFF WHERE S_CITY IS NULL AND S_GRADE IS NOT NULL OR S_GRADE IN (SELECT MIN(S_GRADE) FROM STAFF WHERE EXISTS (SELECT W_HOURS FROM WORKS WHERE W_EMPNUM = 'E3 ')GROUP BY S_CITY) GROUP BY S_CITY, S_GRADE HAVING S_GRADE BETWEEN 11 AND (SELECT MAX(S_GRADE) FROM STAFF WHERE S_EMPNAME > 'AA' AND S_CITY IN (SELECT P_CITY FROM PROJ)) AND S_GRADE BETWEEN 12 AND 18;
