...
SELECT AVG(W_HOURS), W_EMPNUM FROM WORKS WHERE W_PNUM > 'E1 ' AND EXISTS (SELECT * FROM STAFF WHERE S_EMPNUM > 'E3' AND S_GRADE BETWEEN 11 AND 16 AND S_CITY IN (SELECT S_CITY FROM STAFF WHERE S_EMPNUM NOT IN (SELECT W_EMPNUM FROM WORKS WHERE W_HOURS > 30) AND NOT EXISTS (SELECT W_HOURS FROM WORKS WHERE W_EMPNUM = S_EMPNUM) AND S_CITY IS NOT NULL)) GROUP BY W_EMPNUM HAVING EXISTS (SELECT S_GRADE FROM STAFF WHERE W_EMPNUM = S_EMPNUM) AND EXISTS (SELECT S_CITY FROM STAFF WHERE S_EMPNUM > 'E1 ');
