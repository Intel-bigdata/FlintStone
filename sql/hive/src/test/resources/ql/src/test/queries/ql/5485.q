...
SELECT * FROM STAFF, WORKS WHERE S_EMPNUM = W_EMPNUM AND (SELECT MIN(W_HOURS) FROM WORKS) > S_GRADE OR W_PNUM NOT IN (SELECT MIN(P_PNUM) FROM PROJ WHERE S_CITY = P_CITY GROUP BY P_CITY HAVING MAX(P_PNUM) > 'P2 ');
