...
SELECT COUNT(*), W_HOURS, W_EMPNUM, P_PNUM, P_STARTDATE FROM WORKS CROSS JOIN PROJ WHERE W_PNUM = P_PNUM AND P_STARTDATE > DATE '2009-02-03' AND W_EMPNUM IN (SELECT S_EMPNUM FROM STAFF WHERE P_PNUM <> S_EMPNUM) GROUP BY W_HOURS, P_STARTDATE, W_EMPNUM, P_PNUM HAVING P_STARTDATE > DATE '2009-02-03' AND W_EMPNUM IN (SELECT S_EMPNUM FROM STAFF WHERE P_PNUM <> S_EMPNUM);
