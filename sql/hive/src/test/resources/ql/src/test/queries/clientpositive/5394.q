...
SELECT COUNT(*), W_HOURS FROM WORKS CROSS JOIN PROJ WHERE W_PNUM = P_PNUM AND P_ENDDATE BETWEEN (SELECT MAX(P_ENDDATE) FROM PROJ WHERE P_PNUM < 'P6 ') AND DATE '2015-05-06' GROUP BY W_HOURS;
