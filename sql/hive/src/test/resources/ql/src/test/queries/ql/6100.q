...
SELECT AVG(W_HOURS+P_BUDGET/100), W_EMPNUM, W_PNUM FROM WORKS RIGHT OUTER JOIN PROJ ON P_PNUM=W_PNUM WHERE P_ENDDATE BETWEEN DATE '2007-05-06' AND (SELECT MAX(P_ENDDATE) FROM PROJ WHERE P_PNUM < 'P6 ') AND EXISTS (SELECT W_HOURS FROM WORKS LEFT OUTER JOIN PROJ ON P_PNUM=W_PNUM WHERE P_STARTDATE > DATE '2009-02-03' AND P_PNAME NOT IN (SELECT S_EMPNAME FROM STAFF WHERE P_PNUM = S_EMPNUM) GROUP BY W_HOURS) GROUP BY W_EMPNUM, W_PNUM HAVING W_PNUM > 'E1 ' AND EXISTS (SELECT * FROM STAFF WHERE S_EMPNUM > 'E3 ' AND S_GRADE BETWEEN 11 AND 16 AND S_EMPNUM > W_EMPNUM);
