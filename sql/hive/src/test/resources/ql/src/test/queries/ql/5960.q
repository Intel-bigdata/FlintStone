...
SELECT AVG(S_GRADE), COUNT(S_GRADE), S_GRADE FROM STAFF GROUP BY S_GRADE HAVING (SELECT MIN(W_HOURS) FROM WORKS WHERE W_PNUM NOT IN (SELECT P_PNUM FROM PROJ WHERE P_BUDGET < 10000)) > S_GRADE;
