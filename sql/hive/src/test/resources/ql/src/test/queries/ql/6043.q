...
SELECT AVG(W_HOURS), W_EMPNUM, W_PNUM FROM WORKS WHERE W_EMPNUM IN (SELECT S_EMPNUM FROM STAFF) GROUP BY W_EMPNUM, W_PNUM HAVING W_PNUM > (SELECT MIN(P_PNUM) FROM PROJ WHERE P_ENDDATE < DATE '2011-09-14');
