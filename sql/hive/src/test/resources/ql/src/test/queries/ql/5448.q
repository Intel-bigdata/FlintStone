...
SELECT AVG(W_HOURS+P_BUDGET/100), W_EMPNUM FROM WORKS, PROJ WHERE W_PNUM = P_PNUM AND P_CITY IS NOT NULL AND NOT EXISTS (SELECT P_BUDGET FROM PROJ WHERE P_PNAME IS NOT NULL AND W_EMPNUM IS NOT NULL AND P_PNAME = W_EMPNUM AND P_BUDGET IS NOT NULL) GROUP BY W_EMPNUM;
