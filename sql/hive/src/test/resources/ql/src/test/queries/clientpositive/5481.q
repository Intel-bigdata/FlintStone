...
SELECT * FROM STAFF, WORKS WHERE S_EMPNUM = W_EMPNUM AND S_GRADE BETWEEN 11 AND (SELECT MAX(S_GRADE) FROM STAFF WHERE S_EMPNAME > 'AA') OR W_PNUM IN (SELECT P_PNUM FROM PROJ WHERE P_CITY IN (SELECT S_CITY FROM STAFF WHERE S_EMPNAME NOT IN ('A', 'BB', 'EA')));
