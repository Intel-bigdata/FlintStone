...
SELECT COUNT(*), W_HOURS, P_PNAME, P_STARTDATE, P_PNUM FROM WORKS INNER JOIN PROJ ON P_PNUM=W_PNUM GROUP BY W_HOURS, P_PNAME, P_STARTDATE, P_PNUM HAVING P_STARTDATE > DATE '2009-02-03' AND P_PNAME NOT IN (SELECT S_EMPNAME FROM STAFF WHERE P_PNUM = S_EMPNUM);
