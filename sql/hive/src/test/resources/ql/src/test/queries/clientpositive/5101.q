...
select s_empnum, sum(p_budget) from staff join (select p_budget, p_city as pp_city, p_enddate as pp_enddate from proj where substr(p_city, 1, 2) not in (select * from (select substr(p_city, 1, 2) as col1 from proj) p1 where not exists (select * from (select substr(p_city, 0, 5) as col2 from proj) p2 where p2.col2 = p1.col1) and p1.col1 <> 'To')) pproj on s_city = pp_city where s_grade>10 and pp_enddate< date '2011-12-13' group by s_empnum order by s_empnum, sum(p_budget);
