...
SELECT S_GRADE+3*W_HOURS FROM STAFF, WORKS WHERE S_EMPNUM = W_EMPNUM AND S_GRADE < (SELECT MIN(W_HOURS) FROM WORKS WHERE W_PNUM NOT IN (SELECT P_PNUM FROM PROJ WHERE P_STARTDATE != P_ENDDATE));
