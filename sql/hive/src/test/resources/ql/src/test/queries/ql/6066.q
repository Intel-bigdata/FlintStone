...
SELECT COUNT(*), S_CITY, S_GRADE FROM STAFF WHERE S_EMPNUM NOT IN (SELECT W_EMPNUM FROM WORKS WHERE W_HOURS > 30 OR W_PNUM NOT IN (SELECT P_PNUM FROM PROJ WHERE P_PTYPE > 'D')) AND NOT EXISTS (SELECT W_HOURS FROM WORKS WHERE W_EMPNUM = S_EMPNUM) GROUP BY S_GRADE, S_CITY HAVING S_GRADE > (SELECT MIN(W_HOURS) FROM WORKS);
