...
SELECT AVG(S_GRADE), COUNT(S_GRADE), S_GRADE FROM STAFF GROUP BY S_GRADE HAVING S_GRADE > (SELECT MIN(W_HOURS) FROM WORKS WHERE EXISTS (SELECT P_BUDGET FROM PROJ WHERE P_BUDGET BETWEEN 20000.33 AND 80033.24));
