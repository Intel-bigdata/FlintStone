...
SELECT COUNT(*), W_HOURS, P_PNUM, W_EMPNUM, P_STARTDATE FROM WORKS RIGHT OUTER JOIN PROJ ON P_PNUM=W_PNUM GROUP BY W_HOURS, P_PNUM, W_EMPNUM, P_STARTDATE HAVING P_STARTDATE > DATE '2009-02-03' AND W_EMPNUM IN (SELECT S_EMPNUM FROM STAFF WHERE P_PNUM <> S_EMPNUM);
