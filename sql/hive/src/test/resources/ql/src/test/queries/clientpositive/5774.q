...
SELECT AVG(W_HOURS), W_EMPNUM FROM WORKS WHERE W_EMPNUM NOT IN (SELECT S_EMPNUM FROM STAFF WHERE S_GRADE > 13 AND EXISTS (SELECT P_BUDGET FROM PROJ GROUP BY P_BUDGET)) AND EXISTS (SELECT P_BUDGET FROM PROJ WHERE P_PNUM = W_PNUM) GROUP BY W_EMPNUM;
