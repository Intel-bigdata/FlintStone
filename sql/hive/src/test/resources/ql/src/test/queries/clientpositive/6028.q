...
SELECT COUNT(*), S_CITY FROM STAFF WHERE S_GRADE <> (SELECT AVG(W_HOURS) FROM WORKS) GROUP BY S_CITY HAVING (SELECT COUNT(*) FROM STAFF WHERE S_EMPNAME LIKE '___%' AND S_EMPNUM NOT LIKE '_3' AND S_GRADE BETWEEN 11 AND (SELECT MAX(S_GRADE) FROM STAFF WHERE S_EMPNAME > 'AA')) > COUNT(*);
