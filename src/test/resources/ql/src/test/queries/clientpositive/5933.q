...
SELECT COUNT(*) FROM STAFF, WORKS GROUP BY S_EMPNUM HAVING S_EMPNUM NOT IN (SELECT W_EMPNUM FROM WORKS WHERE EXISTS (SELECT P_CITY FROM PROJ WHERE P_CITY NOT IN ('AA', 'BB') GROUP BY P_CITY HAVING P_CITY < 'Y2'));
