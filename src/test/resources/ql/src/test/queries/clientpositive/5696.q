...
SELECT P_BUDGET*2/(P_BUDGET/3) FROM PROJ WHERE P_ENDDATE BETWEEN (SELECT MAX(P_ENDDATE) FROM PROJ WHERE P_PNUM < 'P6 ') AND DATE '2015-05-06' AND NOT EXISTS (SELECT * FROM STAFF, WORKS WHERE S_EMPNAME LIKE '___%' AND S_EMPNUM LIKE '_3 ' AND S_GRADE BETWEEN 11 AND (SELECT MIN(S_GRADE) FROM STAFF WHERE S_EMPNAME > 'AA'));
