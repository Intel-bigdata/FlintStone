...
SELECT COUNT(*), W_HOURS, W_PNUM, P_CITY FROM WORKS JOIN PROJ ON P_PNUM=W_PNUM GROUP BY W_HOURS, W_PNUM, P_CITY HAVING P_CITY IS NOT NULL AND EXISTS (SELECT P_BUDGET FROM PROJ WHERE P_PNUM = W_PNUM);
