...
select s_empnum, s_city from staff s1 where exists (select s_empnum from staff s2 where s1.s_empnum=s2.s_empnum) and not exists (select s_empnum from staff s3 where s1.s_empnum = s3.s_empnum and s1.s_empname = s3.s_empname);
