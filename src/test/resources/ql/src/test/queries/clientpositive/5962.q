...
SELECT AVG(S_GRADE), COUNT(S_GRADE), S_GRADE, S_EMPNUM FROM STAFF GROUP BY S_GRADE, S_EMPNUM HAVING S_GRADE <> (SELECT AVG(W_HOURS) FROM WORKS WHERE S_EMPNUM > W_EMPNUM AND S_GRADE < (SELECT MIN(W_HOURS) FROM WORKS));
