...
SELECT AVG(W_HOURS+P_BUDGET/100), W_EMPNUM, P_PNUM FROM WORKS, PROJ WHERE W_PNUM = P_PNUM AND P_CITY IS NOT NULL AND EXISTS (SELECT P_BUDGET FROM PROJ WHERE P_PNUM = W_PNUM) GROUP BY P_PNUM, W_EMPNUM HAVING W_EMPNUM IN (SELECT S_EMPNUM FROM STAFF WHERE P_PNUM <> S_EMPNUM);
