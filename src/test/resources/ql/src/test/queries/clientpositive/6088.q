...
SELECT AVG(W_HOURS), W_EMPNUM FROM WORKS WHERE W_EMPNUM IN (SELECT S_EMPNUM FROM STAFF WHERE S_CITY NOT IN (SELECT P_CITY FROM PROJ WHERE P_ENDDATE > DATE '2014-08-07')) GROUP BY W_EMPNUM HAVING AVG(W_HOURS) IS NOT NULL AND W_EMPNUM > 'E4 ';
