...
SELECT AVG(W_HOURS), W_EMPNUM, W_PNUM, W_HOURS FROM WORKS WHERE W_EMPNUM NOT IN (SELECT S_EMPNUM FROM STAFF WHERE S_GRADE > 13) AND EXISTS (SELECT P_BUDGET FROM PROJ WHERE P_PNUM = W_PNUM) GROUP BY W_EMPNUM, W_PNUM, W_HOURS HAVING W_PNUM LIKE '%2%' AND W_HOURS IN (SELECT W_HOURS FROM WORKS WHERE W_EMPNUM > 'E5 ');
