...
SELECT COUNT(*), W_HOURS, W_PNUM FROM WORKS CROSS JOIN PROJ WHERE W_PNUM = P_PNUM AND P_ENDDATE BETWEEN DATE '2007-05-06' AND (SELECT MAX(P_ENDDATE) FROM PROJ WHERE P_PNUM < 'P6 ') GROUP BY W_HOURS, W_PNUM HAVING W_HOURS IS NULL AND W_PNUM IS NOT NULL;
