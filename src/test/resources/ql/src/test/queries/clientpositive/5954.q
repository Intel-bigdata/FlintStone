...
SELECT AVG(S_GRADE), COUNT(S_GRADE), S_GRADE FROM STAFF GROUP BY S_GRADE HAVING S_GRADE BETWEEN (SELECT AVG(S_GRADE) FROM STAFF WHERE S_EMPNAME > (SELECT MIN(W_EMPNUM) FROM WORKS WHERE W_HOURS BETWEEN 0 AND 100)) AND 18;
