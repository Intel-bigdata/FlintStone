...
SELECT AVG(W_HOURS), W_EMPNUM FROM WORKS WHERE W_PNUM <> (SELECT MIN(P_PNUM) FROM PROJ WHERE P_ENDDATE < DATE '2011-09-14' AND P_CITY IS NOT NULL AND P_PTYPE IN (SELECT P_PTYPE FROM PROJ WHERE P_STARTDATE < DATE '2012-03-05')) GROUP BY W_EMPNUM;
