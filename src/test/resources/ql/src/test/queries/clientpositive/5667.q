...
SELECT S_CITY FROM STAFF WHERE S_EMPNAME LIKE '___%' AND S_EMPNUM NOT LIKE '_3 ' AND S_GRADE BETWEEN 11 AND (SELECT MAX(S_GRADE) FROM STAFF WHERE S_EMPNAME IN (SELECT MIN(S_EMPNAME) FROM STAFF WHERE S_GRADE > 10 GROUP BY S_CITY HAVING MAX(S_EMPNAME) > 'E2 '));
