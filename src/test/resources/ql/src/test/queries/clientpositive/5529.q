...
SELECT * FROM STAFF, WORKS, PROJ WHERE S_EMPNUM = W_EMPNUM AND P_PNUM = W_PNUM AND (SELECT AVG(P_BUDGET) FROM PROJ WHERE P_PNUM < 'P5 ' OR P_PNUM IN (SELECT MIN(W_PNUM) FROM WORKS WHERE W_HOURS > 20 GROUP BY W_HOURS HAVING MIN(W_PNUM) > 'P1 ')) > P_BUDGET AND S_CITY IS NOT NULL AND S_EMPNUM < 'E4 ';
