...
SELECT AVG(W_HOURS+P_BUDGET/100), W_EMPNUM FROM WORKS, PROJ WHERE W_PNUM = P_PNUM AND W_PNUM LIKE '_6%' AND W_EMPNUM NOT LIKE '%1%' AND P_BUDGET = (SELECT AVG(P_BUDGET) FROM PROJ WHERE P_PNUM = W_PNUM) GROUP BY W_EMPNUM;
