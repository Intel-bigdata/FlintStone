...
SELECT COUNT(*) FROM STAFF, WORKS WHERE S_EMPNUM = W_EMPNUM AND S_GRADE < (SELECT MIN(W_HOURS) FROM WORKS) OR W_PNUM NOT IN (SELECT MIN(P_PNUM) FROM PROJ WHERE S_CITY = P_CITY AND P_PNUM < W_PNUM GROUP BY P_CITY HAVING MAX(P_PNUM) > 'P2 ');
