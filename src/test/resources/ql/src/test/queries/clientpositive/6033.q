...
SELECT COUNT(*), W_HOURS, W_PNUM FROM WORKS CROSS JOIN PROJ WHERE W_PNUM = P_PNUM AND P_ENDDATE BETWEEN (SELECT MAX(P_ENDDATE) FROM PROJ WHERE P_PNUM < 'P6 ') AND DATE '2015-05-06' GROUP BY W_HOURS, W_PNUM HAVING W_HOURS BETWEEN 20 AND 60 AND W_PNUM BETWEEN 'P1 ' AND (SELECT MAX(P_PNUM) FROM PROJ WHERE P_BUDGET > 3000.32);
