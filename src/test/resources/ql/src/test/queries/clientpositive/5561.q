...
SELECT COUNT(*) FROM STAFF, WORKS WHERE S_EMPNUM = W_EMPNUM AND S_EMPNUM NOT IN (SELECT W_EMPNUM FROM WORKS WHERE W_HOURS > 30) AND EXISTS (SELECT W_PNUM, W_HOURS FROM WORKS WHERE W_EMPNUM = S_EMPNUM AND W_PNUM > (SELECT MIN(P_PNUM) FROM PROJ WHERE P_CITY <> 'Deale          ')GROUP BY W_PNUM, W_HOURS HAVING AVG(W_HOURS) < 60) AND S_CITY IS NOT NULL;
