...
SELECT COUNT(*) FROM STAFF, WORKS GROUP BY S_GRADE, W_PNUM, S_CITY HAVING (SELECT MIN(W_HOURS) FROM WORKS) > S_GRADE AND (SELECT AVG(S_GRADE) FROM STAFF) < (SELECT MAX(W_HOURS) FROM WORKS WHERE W_PNUM = 'P3 ') OR W_PNUM NOT IN (SELECT MIN(P_PNUM) FROM PROJ WHERE S_CITY = P_CITY GROUP BY P_CITY HAVING MAX(P_PNUM) > 'P2 ');
