...
SELECT COUNT(*), W_HOURS, P_PNUM, W_PNUM, P_BUDGET, W_EMPNUM FROM WORKS CROSS JOIN PROJ GROUP BY W_HOURS, P_PNUM, W_PNUM, W_EMPNUM, P_BUDGET HAVING W_PNUM = P_PNUM AND W_PNUM LIKE '_6%' AND W_EMPNUM NOT LIKE '%1%' AND P_BUDGET = (SELECT AVG(P_BUDGET) FROM PROJ WHERE P_PNUM = W_PNUM);
