...
SELECT * FROM WORKS INNER JOIN PROJ ON P_PNUM=W_PNUM WHERE P_STARTDATE > DATE '2009-02-03' AND P_PNAME NOT IN (SELECT S_EMPNAME FROM STAFF WHERE P_PNUM = S_EMPNUM AND P_PNUM NOT IN (SELECT W_PNUM FROM WORKS WHERE W_EMPNUM = S_EMPNUM));
