...
SELECT COUNT(*), S_CITY FROM STAFF WHERE S_EMPNUM > 'E1 ' AND EXISTS (SELECT * FROM STAFF WHERE S_EMPNUM > (SELECT MIN(S_EMPNUM) FROM STAFF WHERE S_EMPNUM NOT IN ('E1 ', 'E2 ', 'E3 ', 'E4 ', 'E5 '))) GROUP BY S_CITY HAVING COUNT(*) < (SELECT MIN(S_GRADE) FROM STAFF WHERE S_CITY < 'F4');
