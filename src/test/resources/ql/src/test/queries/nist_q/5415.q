...
SELECT COUNT(*), W_HOURS FROM WORKS LEFT OUTER JOIN PROJ ON P_PNUM=W_PNUM WHERE W_PNUM NOT LIKE '_6%' AND W_EMPNUM NOT LIKE '%1%' AND P_BUDGET = (SELECT AVG(P_BUDGET) FROM PROJ WHERE P_PNUM = W_PNUM) GROUP BY W_HOURS;
