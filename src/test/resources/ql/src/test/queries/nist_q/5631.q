...
SELECT AVG(S_GRADE), COUNT(S_GRADE) FROM STAFF WHERE S_GRADE < (SELECT MIN(W_HOURS) FROM WORKS WHERE W_PNUM NOT IN (SELECT MIN(W_PNUM) FROM WORKS GROUP BY W_EMPNUM HAVING MIN(W_EMPNUM) > 'E3 '));
