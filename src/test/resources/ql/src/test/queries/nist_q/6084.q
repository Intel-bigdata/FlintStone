...
SELECT AVG(W_HOURS), W_EMPNUM FROM WORKS WHERE 'E1 ' < W_PNUM OR NOT EXISTS (SELECT * FROM STAFF WHERE S_EMPNUM > 'E3' AND S_GRADE BETWEEN 11 AND 16 AND S_CITY > (SELECT MAX(S_CITY) FROM STAFF WHERE S_EMPNUM NOT IN (SELECT W_EMPNUM FROM WORKS WHERE W_HOURS > 30) AND EXISTS (SELECT W_HOURS FROM WORKS WHERE W_EMPNUM = S_EMPNUM) AND S_CITY IS NOT NULL)) GROUP BY W_EMPNUM HAVING EXISTS (SELECT S_EMPNUM FROM STAFF, WORKS WHERE S_EMPNUM = W_EMPNUM GROUP BY S_EMPNUM  HAVING EXISTS (SELECT W_HOURS FROM WORKS WHERE W_EMPNUM = S_EMPNUM) AND S_EMPNUM > 'E1 ');
