...
SELECT AVG(W_HOURS), W_EMPNUM FROM WORKS WHERE W_PNUM NOT LIKE '%3%' AND W_HOURS > (SELECT MIN(W_HOURS) FROM WORKS WHERE W_EMPNUM > (SELECT MIN(S_EMPNUM) FROM STAFF WHERE S_EMPNAME > 'C1 ')) GROUP BY W_EMPNUM HAVING AVG(W_HOURS) > (SELECT AVG(S_GRADE) FROM STAFF WHERE STAFF.S_CITY NOT IN ('AA', 'GB')) AND W_EMPNUM BETWEEN 'E2 ' AND 'E5 ';
