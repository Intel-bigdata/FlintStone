...
SELECT COUNT(*), S_CITY, S_EMPNUM FROM STAFF WHERE S_EMPNUM IN (SELECT W_EMPNUM FROM WORKS WHERE W_EMPNUM NOT IN (SELECT S_EMPNUM FROM STAFF WHERE S_CITY NOT IN ('AA','BB','Deale          '))) GROUP BY S_CITY, S_EMPNUM HAVING S_EMPNUM IN (SELECT W_EMPNUM FROM WORKS WHERE W_HOURS > 30) AND EXISTS (SELECT W_HOURS FROM WORKS WHERE W_EMPNUM = S_EMPNUM);
