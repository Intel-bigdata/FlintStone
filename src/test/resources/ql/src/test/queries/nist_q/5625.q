...
SELECT AVG(S_GRADE), COUNT(S_GRADE) FROM STAFF WHERE S_GRADE BETWEEN (SELECT MAX(S_GRADE) FROM STAFF WHERE S_EMPNAME > (SELECT MIN(W_EMPNUM) FROM WORKS WHERE W_HOURS BETWEEN 0 AND 100)) AND (SELECT MAX(S_GRADE) FROM STAFF WHERE S_EMPNAME > 'AA');
