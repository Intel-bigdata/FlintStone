...
SELECT AVG(W_HOURS+P_BUDGET/100), W_EMPNUM, P_STARTDATE, P_PNAME, P_PNUM FROM WORKS, PROJ WHERE W_PNUM = P_PNUM GROUP BY W_EMPNUM, P_STARTDATE, P_PNAME, P_PNUM HAVING P_STARTDATE > DATE '2009-02-03' AND W_EMPNUM IN (SELECT S_EMPNUM FROM STAFF WHERE P_PNUM <> S_EMPNUM);
