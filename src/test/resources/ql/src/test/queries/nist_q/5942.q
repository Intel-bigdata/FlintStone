...
SELECT COUNT(*) FROM STAFF, WORKS GROUP BY S_GRADE, W_PNUM, S_CITY HAVING S_GRADE < (SELECT MIN(W_HOURS) FROM WORKS) OR W_PNUM NOT IN (SELECT MIN(P_PNUM) FROM PROJ WHERE S_CITY = P_CITY GROUP BY P_CITY HAVING MAX(P_PNUM) > 'P2 ');
