...
SELECT S_CITY FROM STAFF WHERE S_EMPNUM NOT IN (SELECT W_EMPNUM FROM WORKS WHERE W_HOURS > 30) AND EXISTS (SELECT W_HOURS FROM WORKS WHERE W_EMPNUM = S_EMPNUM) AND S_CITY IS NOT NULL;
