...
SELECT COUNT(*) FROM STAFF, WORKS GROUP BY S_EMPNUM HAVING S_EMPNUM NOT IN (SELECT W_EMPNUM FROM WORKS WHERE W_HOURS > 30 AND W_PNUM IN (SELECT P_PNUM FROM PROJ WHERE P_CITY <> 'Deale          ' AND P_PTYPE IN ('Design', 'Test  '))) AND EXISTS (SELECT W_HOURS FROM WORKS WHERE W_EMPNUM = S_EMPNUM);
