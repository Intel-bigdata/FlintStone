...
SELECT COUNT(*), S_CITY, S_EMPNUM FROM STAFF WHERE S_GRADE < (SELECT MIN(W_HOURS) FROM WORKS) GROUP BY S_CITY, S_EMPNUM HAVING S_EMPNUM <> 'E1 ' AND EXISTS (SELECT * FROM STAFF WHERE S_EMPNUM > 'E8 ');
