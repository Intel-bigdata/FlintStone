...
SELECT AVG(W_HOURS+P_BUDGET/100), W_EMPNUM, W_HOURS, W_PNUM FROM WORKS, PROJ WHERE W_PNUM = P_PNUM AND P_CITY IS NOT NULL AND NOT EXISTS (SELECT P_BUDGET FROM PROJ WHERE P_PNAME IS NOT NULL AND W_EMPNUM IS NOT NULL AND P_PNAME = W_EMPNUM AND P_BUDGET IS NOT NULL) GROUP BY W_PNUM, W_HOURS, W_EMPNUM HAVING W_HOURS BETWEEN 20 AND 60 AND W_PNUM BETWEEN 'P1 ' AND (SELECT MAX(P_PNUM) FROM PROJ WHERE P_BUDGET > 3000.32);
