...
SELECT COUNT(*) FROM STAFF, WORKS GROUP BY S_EMPNUM, S_CITY HAVING S_EMPNUM NOT IN (SELECT W_EMPNUM FROM WORKS WHERE W_HOURS > 30) AND EXISTS (SELECT W_PNUM FROM WORKS WHERE W_EMPNUM = S_EMPNUM AND W_PNUM > (SELECT MIN(P_PNUM) FROM PROJ WHERE P_CITY <> 'Deale          ')GROUP BY W_PNUM HAVING AVG(W_HOURS) < 60) AND S_CITY IS NOT NULL;
