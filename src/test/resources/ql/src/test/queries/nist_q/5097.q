...
select sum(case when p_budget>20000.0 or p_ptype like 'tes%' then 0.0 else p_budget end) from proj group by p_city, p_ptype order by sum(case when p_budget>20000.0 or p_ptype like 'tes%' then 0.0 else p_budget end);
