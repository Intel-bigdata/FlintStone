...
SELECT COUNT(*), W_HOURS, P_ENDDATE FROM WORKS CROSS JOIN PROJ WHERE W_PNUM = P_PNUM AND W_PNUM LIKE '_6%' AND W_EMPNUM NOT LIKE '%1%' AND P_BUDGET = (SELECT AVG(P_BUDGET) FROM PROJ WHERE P_PNUM = W_PNUM) GROUP BY P_ENDDATE, W_HOURS HAVING P_ENDDATE BETWEEN DATE '2007-05-06' AND (SELECT MAX(P_ENDDATE) FROM PROJ WHERE P_PNUM < 'P6 ');
