...
SELECT AVG(W_HOURS+P_BUDGET/100), W_EMPNUM, P_ENDDATE FROM WORKS, PROJ WHERE W_PNUM = P_PNUM GROUP BY W_EMPNUM, P_ENDDATE HAVING P_ENDDATE BETWEEN DATE '2007-05-06' AND (SELECT MAX(P_ENDDATE) FROM PROJ WHERE P_PNUM < 'P6 ');
