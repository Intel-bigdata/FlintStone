...
SELECT AVG(S_GRADE), COUNT(S_GRADE), S_GRADE FROM STAFF GROUP BY S_GRADE HAVING S_GRADE NOT BETWEEN 11 AND (SELECT MAX(S_GRADE) FROM STAFF WHERE S_EMPNAME > (SELECT MIN(W_EMPNUM) FROM WORKS WHERE W_HOURS > 30.0));
