...
select s_empnum, sum(p_budget) from staff join (select p_budget, p_city as pp_city, p_enddate as pp_enddate from proj) pproj on s_city = pp_city where s_grade>10 and pp_enddate< date '2011-12-13' group by s_empnum;
